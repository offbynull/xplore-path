...
class EvaluatorTest(unittest.TestCase):
    ...
    def test_must_walk_to_self_using_directive(self):
        root = PythonObjectPath.create_root_path({'a': {'b': {'c': 1, 'd': 2, 'e': -1, 'f': -2}}, 'y': 3, 'z': 4, 'ptrs': {'d_ptr': 'd', 'f_ptr': 'f'}})
    ...
    def test_must_walk_to_self_using_directive_combined_with_dot(self):
        root = PythonObjectPath.create_root_path({'a': {'b': {'c': 1, 'd': 2, 'e': -1, 'f': -2}}, 'y': 3, 'z': 4, 'ptrs': {'d_ptr': 'd', 'f_ptr': 'f'}})
        self.assertEqual([e.value() for e in evaluate(root, '/a/self::(.)')], [root.value()['a']])
        self.assertEqual([e.value() for e in evaluate(root, '/a/b/self::(.)')], [root.value()['a']['b']])
        self.assertEqual([e.value() for e in evaluate(root, '/a/b/c/self::(.)')], [root.value()['a']['b']['c']])
        self.assertEqual([e.value() for e in evaluate(root, '/a/b/d/self::(.)')], [root.value()['a']['b']['d']])
        self.assertEqual([e.value() for e in evaluate(root, '/a/b/xxx/self::(.)')], [])
