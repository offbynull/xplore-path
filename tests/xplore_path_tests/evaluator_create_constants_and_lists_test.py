...
class EvaluatorTest(unittest.TestCase):
    ...
    def test_must_produce_matchers(self):
        self.assertIsInstance(evaluate(DummyPath(), '*').single.value, WildcardMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 's"hello"').single.value, StrictMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 's\'hello\'').single.value, StrictMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'r"hello"').single.value, RegexMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'r\'hello\'').single.value, RegexMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'f"hello"').single.value, FuzzyMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'f\'hello\'').single.value, FuzzyMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'g"hello"').single.value, GlobMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'g\'hello\'').single.value, GlobMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'i"hello"').single.value, IgnoreCaseMatcher)
        self.assertIsInstance(evaluate(DummyPath(), 'i\'hello\'').single.value, IgnoreCaseMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~1:5').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~[1:5]').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~[1:5)').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~(1:5]').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~(1:5)').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~1').single.value, NumericRangeMatcher)
        self.assertIsInstance(evaluate(DummyPath(), '~1@0.001').single.value, NumericRangeMatcher)
