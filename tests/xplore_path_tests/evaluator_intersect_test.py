...
class EvaluatorTest(unittest.TestCase):
    ...
    def test_must_intersect_seq_vs_seq(self):
        self.assertEqual(sorted(evaluate(DummyPath(), '(5,7,9) intersect (5,1,9)').unpack), [5, 9])
        self.assertEqual(sorted(evaluate(DummyPath(), '(5,7,9) intersect (5,1,"9")').unpack), [5])  # THIS IS WHAT XPATH DOES - no coercing before matching - intersects with just data and type
        self.assertEqual(sorted(evaluate(DummyPath(), '(5,7,9) intersect ()').unpack), [])
        self.assertEqual(sorted(evaluate(DummyPath(), '() intersect (5,7,9)').unpack), [])
