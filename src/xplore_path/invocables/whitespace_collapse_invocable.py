...
class WhitespaceCollapseInvocable(Invocable):
    """
    ```Invocable`` that scans over ``Entity`` values in a ``Collection`` (``Node``\s unpacked to their values) and
    collapses stretches of contiguous whitespace into a single space. Non-string values are coerced to string, being
    silently discarded if coercion is not possible.
    """
